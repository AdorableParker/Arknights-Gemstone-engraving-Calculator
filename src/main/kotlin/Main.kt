...
fun main() {
    ...
    val solver = Solver(
        10,70,10,10, 5,  // 火 叶 空 沙 四项原料 以及 操作台数量
        // 输入拥有工艺
    ...
    val (materialArray, processStepsMax, craftAreaArray, craftConsoleArray) = solver.optimalProcess() // 获取最优结果数据
    println(craftAreaArray.joinToString(",", "工艺区: [", "]") { "${it.name}-${it.level}" })
    println(craftConsoleArray.joinToString(",", "操作台: [", "]") { "${it.name}-${it.level}" })

    // 最优结果流程
    InerSet.resetState(materialArray, processStepsMax, craftAreaArray, craftConsoleArray)
